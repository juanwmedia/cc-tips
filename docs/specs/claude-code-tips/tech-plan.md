...
### B6 — 2026-05-01 — Repo restructure: tips/ moved out of plugin install
- **Problem**: After publishing, `/plugin install` cloned the entire repo into `~/.claude/plugins/cache/.../<version>/`, including all 110 tip markdowns at the install root. Two costs: install size grew unnecessarily (~220 KB of tips not yet requested), and every tip-content fix forced users through `/plugin marketplace update` to see it — even though `manifest.json`'s `url_es` / `url_en` URLs already serve the latest content from `main`.
- **Decision**: physically separate the plugin source from the tip content. Created a `plugin/` subdirectory and moved `.claude-plugin/plugin.json`, `skills/`, `hooks/`, and `manifest.json` into it. `tips/` stays at the repo root, **outside** the plugin install. `marketplace.json` (still at the repo root, since the repo IS the marketplace) now points its `source` field to `./plugin` instead of `./`.
- **Effect on `open`**: no behavior change. The skill already curls from `raw.githubusercontent.com/juanwmedia/cc-tips/main/tips/...` and caches under `${CLAUDE_PLUGIN_DATA}/tips/<slug>-<lang>-v<version>.md`. After this restructure that path is the *only* path — there is no in-plugin `tips/` to fall back on, which matches what the skill body was already describing.
